...
//				contentRect.xMax += marginLeft;
		...
		if (isScrollWheelEvent)
		{
			scrollPosition.x = Mathf.Clamp(scrollPosition.x + Event.current.delta.x * charSize.y, 0f, contentRect.width - scrollViewRect.width);
			scrollPosition.y = Mathf.Clamp(scrollPosition.y + Event.current.delta.y * charSize.y, 0f, contentRect.height - scrollViewRect.height);
			Event.current.Use();
